...
bool MfccMelFilterbank::Initialize(int input_length, double input_sample_rate,
  ...
  if (num_channels_ < 1) {
    cout << "Number of filterbank channels must be positive." << endl;
    return false;
  }

  if (sample_rate_ <= 0) {
    cout << "Sample rate must be positive." << endl;
    return false;
  }

  if (input_length < 2) {
    cout << "Input length must greater than 1." << endl;
    return false;
  }

  if (lower_frequency_limit < 0) {
    cout << "Lower frequency limit must be nonnegative." << endl;
    return false;
  }

  if (upper_frequency_limit <= lower_frequency_limit) {
    cout << "Upper frequency limit must be greater than "
               << "lower frequency limit." << endl;
    return false;
  }
  ...
  if (!bad_channels.empty()) {
    cout << "Missing " << bad_channels.size() << " bands "
               ...
               << " upper_frequency_limit: " << upper_frequency_limit << endl;
  }
  initialized_ = true;
  return true;
}
...
void MfccMelFilterbank::Compute(const std::vector<double> &input,
  ...
  if (!initialized_) {
    cout << "Mel Filterbank not initialized." << endl;
    return;
  }

  if (input.size() <= end_index_) {
    cout << "Input too short to compute filterbank" << endl;
    return;
  }
