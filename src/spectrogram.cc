...
bool Spectrogram::Initialize(const std::vector<double>& window, int step_length) {
  ...
  if (window_length_ < 2) {
    cout << "Window length too short." << endl;
    initialized_ = false;
    return false;
  }
  ...
  if (step_length_ < 1) {
    cout << "Step length must be positive." << endl;
    initialized_ = false;
    return false;
  }
...
bool Spectrogram::ComputeComplexSpectrogram(
  ...
  if (!initialized_) {
    cout << "ComputeComplexSpectrogram() called before successful call "
               << "to Initialize().";
    return false;
  }
  // CHECK(output);
  ...
  while (GetNextWindowOfSamples(input, &input_start)) {
    ...
    std::vector<complex<OutputSample> >& spectrogram_slice = output->back();
...
bool Spectrogram::ComputeSquaredMagnitudeSpectrogram(
  ...
  if (!initialized_) {
    cout << "ComputeSquaredMagnitudeSpectrogram() called before "
  ...
  while (GetNextWindowOfSamples(input, &input_start)) {
    ...
    output->resize(output->size() + 1);
    // Get a reference to the newly added slice to fill in.
    std::vector<OutputSample>& spectrogram_slice = output->back();
    spectrogram_slice.resize(output_frequency_channels_);
    for (int i = 0; i < output_frequency_channels_; ++i) {
...
bool Spectrogram::GetNextWindowOfSamples(const std::vector<InputSample>& input, int* input_start) {
  auto input_it = input.begin() + *input_start;
  // input_it += *input_start;
